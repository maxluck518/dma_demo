...
extern "C" void FiveTupleDemo_DPI( 
	...
	int   _bozo2  // dont care                  
		...
		// engine instance
		FiveTupleDemo _DUT("FiveTupleDemo");
		...
		for (int _p = 0; _p < _packets.size(); _p++) {
			...
			_DUT.metadata_in = _LV<128>(_tuples[_p][0], 16);
			// format input and inout tuples
			_tuple_in_stream << ((_LV<128>)_DUT.metadata_in).to_hex() << ' ';
			...
			_tuple_out_stream << ((_LV<128>)_DUT.metadata_out).to_hex() << ' ';
			_tuple_out_stream << ((_LV<16>)_DUT.counter_index_out).to_hex() << ' ';
